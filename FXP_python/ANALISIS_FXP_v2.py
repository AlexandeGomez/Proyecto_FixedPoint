...
#Guardar los valores del coseno en un txt
listatotxt = cos_cuan.bin()
with open("datacoseno.txt", "w") as archivotxt:
    for i in range(len(listatotxt)):
        archivotxt.write(listatotxt[i]+"\n")


#%% 
InOutData = []
for i in range(len(cos_cuan)):
    InOutData.append([cos_cuan[i].bin(), dataout[i].bin()])
IO = np.array(InOutData)
